...
if __name__ == "__main__":
    ...
    ticket_tool = next((t for t in tools if t.get("name") == "register_monthly_ticket"), None)
    if ticket_tool and not ticket_tool.get("parameters", {}).get("properties"):
    ...
    test_result = server.call_tool("route_query", {"route_id": "VB01"})
    if not test_result:
        print("⏳ [TODO 2.1]: Hàm call_tool() đang trả về rỗng. Học viên hãy hoàn thiện TODO 2.1 trong 'src/mcp_server.py'!")
    else:
        print(f"✅ [TODO 2.1]: Test dispatch tool 'route_query' thành công:")
